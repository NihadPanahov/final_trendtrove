...
const BlogCard = ({ blog }) => {
  ...
  const containerClass = `flex flex-wrap items-center bg-gray-300 shadow-md rounded-[20%] p-6 mb-4 transform hover:translate-x-[50px] hover:scale-105 transition-transform duration-300 cursor-default my-10 ${
    ...
    <div className={containerClass}>
      {isImageOnLeft && (
        <div className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 mb-4 md:mb-0">
          <img src={image} alt={title} className="w-full h-auto rounded-[70px]" />
        </div>
      )}
